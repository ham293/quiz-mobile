...
| 版本 | 说明 | 下载 |
| --- | --- | --- |
| `app-release.apk`（6.0 MB） | **推荐**，v1.1.0 | [Release 页面](https://github.com/ham293/quiz-mobile/releases/latest) 或 [直接下载](https://github.com/ham293/quiz-mobile/releases/download/v1.1.0/app-release.apk) |
| `app-debug.apk`（7.1 MB） | 带调试信息，装不上时换这个试试 | [直接下载](https://github.com/ham293/quiz-mobile/releases/download/v1.1.0/app-debug.apk) |

> **手机网络打不开 github.com 下载链接时**（国内常见），在下载地址前面加一个加速前缀即可，例如：
> `https://ghproxy.net/https://github.com/ham293/quiz-mobile/releases/download/v1.1.0/app-release.apk`
...
3. **下载安装**：
   ...
   - 更方便的做法：打 tag 并推送，工作流会把 APK 直接发布到 **Releases**（本项目已发布 v1.1.0）：
     ```bash
...
| **AI 识别题目（可选）** | 排版特殊的题库（双栏、串行、选项挤在一起…）规则解析难免出错时，可改用大模型识别：设置 → AI 识别设置里填自己的 API Key，导入时点「🤖 用 AI 识别题库」。**只有你主动点它才会联网**，本地规则解析始终可用作回退 |

截图见 `docs/screenshots/`。

### AI 识别怎么用（需要自己的 API Key）

内置了这些服务商（都走 OpenAI 兼容接口，地址与默认模型已预填）：

| 服务商 | 说明 | 申请 Key |
| --- | --- | --- |
| **智谱 GLM-4-Flash**（默认推荐） | **免费额度**，个人用足够 | https://open.bigmodel.cn/usercenter/apikeys |
| 硅基流动 SiliconFlow | 有免费模型 | https://cloud.siliconflow.cn/account/ak |
| Kimi（月之暗面） | 质量好，新用户有赠送额度 | https://platform.moonshot.cn/console/api-keys |
| 豆包（火山方舟） | 同上 | https://console.volcengine.com/ark |
| DeepSeek | 便宜、稳定 | https://platform.deepseek.com/api_keys |
| 自定义 | 任何 OpenAI 兼容接口 | — |

用法：**设置 → AI 识别设置** → 选服务商 → 粘贴 API Key → 「测试连接」→ 回到题库页点 **🤖 用 AI 识别题库**。
导入时会分块发送题库文本（默认每块 6000 字），显示「AI 识别中 第 x/y 块」，识别完可预览、删除不要的题，再存成新题库。

> ⚠️ 两点提醒：
> 1. **API Key 是你自己的账号凭据，只存在手机本地**（localStorage），不要发给任何人；本 App 也不会把它上传到除你选定的服务商之外的任何地方。
> 2. **用 AI 识别会把该题库的文本上传到所选服务商的服务器**；不想上传就用默认的本地规则解析（完全离线）。
